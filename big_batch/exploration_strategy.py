...
class ExplorationStrategy:
    ...
    def get_all_pending_explorations(self) -> List[List[int]]:
        """Get ALL paths that need to be explored, prioritized and ready for batch execution"""
        all_paths = []
        
        # Collect all exploration paths by priority
        
        # Priority 1: Unknown connections in complete rooms (batch mode)
        unknown_connections = self.get_unknown_connections_to_verify()
        for connection in unknown_connections:
            if connection["priority"] == "complete_blocking_partial_room_batch":
                # This gives us multiple paths to explore at once
                all_paths.extend(connection["paths"])
            else:
                all_paths.append(connection["path"])
        
        # Priority 2: Missing connections from complete rooms
        missing_connections = self.get_missing_connections_from_complete_rooms()
        for connection in missing_connections:
            all_paths.append(connection["path"])
        
        # Priority 3: Partial room explorations
        partial_explorations = self.get_partial_rooms_to_explore()
        for exploration in partial_explorations:
            all_paths.append(exploration["path"])
        
        # Priority 4: All doors from incomplete rooms
        incomplete_rooms = self.room_manager.get_incomplete_rooms()
        for room in incomplete_rooms:
            if not room.paths:
                continue
            ...
            if doors_to_explore:
                ...
                for door in doors_to_explore:
                    plan = base_path + [door]
                    all_paths.append(plan)
        
        # Filter out already explored paths
        filtered_paths = []
        for path in all_paths:
            if self.should_explore_path(path):
